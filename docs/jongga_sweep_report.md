...
## Phase 3: nopen_gap1 옵션화 + 수급 보강 (2026-05-05)

### A. `sw_nopen_gap1` 옵션 운영 적용

백테 결과상 MDD 기준 sw_pe_t8(-53%) 보다 양호한 sw_nopen_gap1(MDD -48%, Sharpe 3.23, EV +2.37%).
기본은 close 모드(기존)이며 옵션으로 next_open + 갭 1% 필터 사용.

**구현**:
- `engine/config.py` : `entry_timing` ("close"/"next_open"), `max_gap_pct` 필드 추가
- `signal_tracker.track_signals(entry_timing=, max_gap_pct=)` :
  - `pending` 상태 + next_open 모드 → 다음 거래일 시가 갭 검증
  - 갭 ≤ 1% : `entry_price` 시가로 갱신, `status=entered`
  - 갭 > 1% : `status=invalidated`, `exit_reason=gap_skip`
- 진입 당일은 trailing skip (다음 호출부터 본격 trailing)

**테스트 보강**: `tests/test_signal_tracker.py::TestNextOpenEntry` 4건 추가
- 다음 거래일 데이터 없으면 pending 유지
- 갭 이내면 시가 진입 + entry_price 갱신
- 갭 초과면 invalidated + gap_skip
- close 모드 무회귀

### B. 수급(외인/기관) KOSDAQ 보강

기존 `KRXCollector.get_supply_data` 가 KOSPI 만 호출 → KOSDAQ 종목은 항상 supply=0.
이 누락이 수급 점수(0~2점) 제외 효과를 발생시켜 KOSDAQ 픽 EV 가 저평가됐을 가능성.

**구현**: `engine/collectors.py` : KOSPI 우선 조회 → 미발견 시 KOSDAQ 재조회.

### C. 미해결 (운영 결정 필요)

1. **Historical supply 백필 + 백테 연동**
   - 백테 score 계산은 수급 점수 미적용 상태
   - 1~2년치 일별 외인/기관 net buy 를 SQLite/Parquet 에 backfill 후 score 재계산
   - 추정 비용: pykrx API ~500일 × 2시장 = 1000회 (약 30분)
   - 비용 대비 EV 개선 폭 불확실 → 사용자 결정 영역

2. **paper_trading 라이브 검증**
   - sw_pe_t8 + nopen_gap1 옵션 모두 라이브 모드에서 1~2주 검증 필요
   - 코드는 이미 호환, 운영 사이클(매일 09:00 / 15:35) 만 돌리면 됨
